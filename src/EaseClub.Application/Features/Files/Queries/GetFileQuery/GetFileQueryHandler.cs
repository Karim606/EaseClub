...
namespace EaseClub.Application.Features.Files.Queries.GetFileQuery
{
    public class GetFileQueryHandler(IFileRepository fileRepository,
        ...
        ILogger<GetFileQueryHandler> logger) : IRequestHandler<GetFileQuery, Result<FileDto>>
    {
        ...
        public async Task<Result<FileDto>> Handle(GetFileQuery request, CancellationToken ct)
        {
            ...
            if (file == null) { logger.LogError("NotFound error in GetFileQueryHandler: {Error}", Error.NotFound("File not found").ToLogObject()); return Error.NotFound("File not found"); }
            var canAccess = await fileAuthorizationService.CanAccessAsync(file);

                if (!canAccess) { logger.LogError("Unauthorized error in GetFileQueryHandler: {Error}", Error.Unauthorized("You do not have permission to access this file").ToLogObject()); return Error.Unauthorized("You do not have permission to access this file"); }
